...
class RecommendedItem {
  ...
  def Recommend(items_similar: RDD[ItemSimi], user_prefer: RDD[ItemPref], r_number: Int): (RDD[UserRecomm]) = {
    ...
    //3.矩阵计算（i行j列相乘）
    val rdd_app1_R3: RDD[((String, String), Double)] = rdd_app1_R2.map(f => ((f._2._2._1, f._2._1._1), 1/(Math.pow(f._2._2._2,f._2._1._2))))
    //4.矩阵计算（用户：元素累加求和）
  ...
  用户推荐计算
   */
  def Recommend1(items_similar: RDD[ItemSimi1], user_prefer: RDD[ItemPref1], r_number: Int): (RDD[UserRecomm1]) = {
    //1.数据准备
    val rdd_app1_R1: RDD[(Int, Int, Double)] = items_similar.map(f => (f.itemId1, f.itemId2, f.similar))
    val user_prefer1: RDD[(Int, Int, Double)] = user_prefer.map(f => (f.userId, f.itemId, f.pref))
    //2.矩阵计算（i行j列join）
    val rdd_app1_R2: RDD[(Int, ((Int, Double), (Int, Double)))] = rdd_app1_R1.map(f => (f._1, (f._2, f._3))).join(user_prefer1.map(f => (f._2, (f._1, f._3))))
    //3.矩阵计算（i行j列相乘）
    val rdd_app1_R3: RDD[((Int, Int), Double)] = rdd_app1_R2.map(f => ((f._2._2._1.toInt, f._2._1._1.toInt), 1/(Math.pow(f._2._2._2,f._2._1._2))))
    //4.矩阵计算（用户：元素累加求和）
    val rdd_app1_R4: RDD[((Int, Int), Double)] = rdd_app1_R3.reduceByKey((x, y) => x + y)
    //5.矩阵计算（用户：对结果过滤已有物品）
    val rdd_app1_R5: RDD[(Int, (Int, Double))] = rdd_app1_R4.leftOuterJoin(user_prefer1.map(f => ((f._1, f._2), 1))).filter(f => f._2._2.isEmpty).map(f => (f._1._1, (f._1._2, f._2._1)))
    //6.矩阵计算（用户：用户对结果排序，过滤）
    val rdd_app1_R6: RDD[(Int, Iterable[(Int, Double)])] = rdd_app1_R5.groupByKey()
    val rdd_app1_R7: RDD[(Int, Iterable[(Int, Double)])] = rdd_app1_R6.map(f => {
      val i2: mutable.Buffer[(Int, Double)] = f._2.toBuffer
      val i2_2: mutable.Buffer[(Int, Double)] = i2.sortBy(_._2)
      if (i2_2.length > r_number) i2_2.remove(0, (i2_2.length - r_number))
      (f._1, i2_2.toIterable)
    })
    val rdd_app1_R8: RDD[(Int, Int, Double)] = rdd_app1_R7.flatMap(f => {
      val id2: Iterable[(Int, Double)] = f._2
      for (w <- id2) yield (f._1, w._1, w._2)
    })
    rdd_app1_R8.map(f => UserRecomm1(f._1, f._2, f._3))
  }
  /**
   * 用户推荐计算
   *
